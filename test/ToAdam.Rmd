...
for (name_herbicide in list_herbicides){
  ...
  if (sum(!is.na(subdat_resistance))==0){
    ...
    par(fig=c(0,1,0,.8), mar=c(rep(3,4)), xpd=T, new=F)
    hist(resistance, col=rgb(0.5,0.5,0.5,alpha=0.6), freq=T, breaks=10, bord=FALSE, main="", xlim = c(0,1), xlab=name_herbicide, ylim = c(0,10))
    ...
    legend(x=0.4, y=10, title="The herbicide resistance situation of your farm in ", legend=paste0(subdat$Location[!is.na(subdat_resistance)], " is lower (better) than other ", round(list_percentile[!is.na(subdat_resistance)]*100), "% farms"), col=list_colors[!is.na(subdat_resistance)], pch=c(19,19), box.col = F, cex=.9)
  ...
  par(fig=c(0,0.9,0,1), mar=c(1,1,1,1), xpd=F)
  ...
  ### overlay the heatmap
  par(fig=c(0,0.9,0,1), mar=c(1,1,1,1), xpd=F, new=TRUE)
  ...
  par(fig=c(0.9,1,0.2,0.8), mar=c(1,1,1,1), xpd=F, new=TRUE)
